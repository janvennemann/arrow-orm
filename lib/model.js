...
function Model(name, definition, skipValidation) {
	...
	var ModelFields = ['name', 'fields', 'connector', 'metadata', 'mappings', 'actions', 'disabledActions', 'singular', 'plural', 'autogen', 'generated', 'cache'];
	...
	if (this.cache === true || _.isPlainObject(this.cache)) {
...
Model.prototype.findOneAPI = function findOneAPI() {
	return {
		...
		nickname: 'Find One',
		method: 'GET',
		description: this.findOneDescription || 'Find one ' + this.singular,
...
Model.prototype.findOne = function findOne(id, callback) {
	try {
		...
		this.getConnector()[_.isArray(id) ? 'findOneMany' : 'findOne'](this, id, next);
	} catch (E) {
		return callback(E);
	}
};
