...
var Capabilities = exports.Capabilities = {
	...
	/**
	 * Specifies that the connector provides a HTML form that allows to configure
	 * the connector through Arrow Admin.
	 */
	ProvidesWebConfiguration: 'ProvidesWebConfiguration'
};
...
var validations = {
	...
	ProvidesWebConfiguration: function (impl) {
		// FIXME: Is there any better/faster test we can do here?
		var connectorDir = path.resolve(path.join(impl.filename, '..', '..'));
		var allRequiredFilesPresent = fs.existsSync(path.join(connectorDir, 'web/configuration.js')) && fs.existsSync(path.join(connectorDir, 'web/configuration.html'));
		return allRequiredFilesPresent && impl.getConfigurationView && impl.getConfigurationScript;
	}
};
...
function crawlTemplates(connectorDir, templateDir, currentPath) {
	...
	for (var i = 0; i < children.length; i++) {
		...
		else if (child.slice(-3) === '.js' || child.slice(-5) === '.html') {
