...
function Connector(impl, config) {
	impl && _.merge(this,_.omit(impl,'connect', 'loginRequired'));
	...
	if (impl && _.isFunction(impl.loginRequired)) {
		this._loginRequired = impl.loginRequired;
	}
...
Connector.prototype.loginRequired = function (callback) {
	return this._loginRequired(this.request, callback);
};
		...
		// we're not connected, call through to connect before continuing
			...
			if (self.loginRequired) {
				self.loginRequired(function (err, required) {
					...
					if (required) {
						self.login(model.request, model.response, function (err) {
							if (err) { return callback(err); }
							connect();
						});
					} else {
 ...
 * @param {Function} callback Callback passed an Error object (or null if successful).
 */
Connector.prototype.findOneMany = function findOneMany(Model, ids, callback) {
	async.mapLimit(ids, 4, Model.findOne.bind(Model), callback);
};
