...
function Connector(impl, config) {
	...
	if (impl && impl.capabilities) {
		this.capabilities = impl.capabilities;
	}
...
/**
 * Returns wether or not this connector supports a given capability
 *
 * @param {string} capability One of the Connector.Capability.* constants
 * @return {boolean}
 */
Connector.prototype.hasCapability = function(capability) {
	if (!this.capabilities) {
		this.logger && this.logger.trace('No capabilities for ' + this.name + ' connector stored, hasCapability will always return false.');
		return false;
	}
	return _.includes(this.capabilities, capability);
};
